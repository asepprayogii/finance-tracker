...
import { SlidersHorizontal, List, TrendingUp, TrendingDown, Pencil, Trash2, FileDown, X, Check, Calendar, ChevronDown, ChevronUp, Filter, ArrowLeft, ArrowRight, RefreshCw, Clock } from "lucide-react";
...
export default function Transactions() {
  ...
  async function fetchCategories() {
    try {
      ...
      let query = supabase.from("categories").select("id, name, type, icon").or(`user_id.eq.${user.id},is_default.eq.true`);
  ...
  async function fetchTransactions() {
    ...
    try {
      ...
      let query = supabase.from("transactions").select("*, categories(name, icon), created_at").eq("user_id", user.id).order("date", { ascending: false }).order("created_at", { ascending: false });
  ...
  const activeFiltersCount = [filterType !== "all", filterCategory !== "all", filterStartDate !== defaultStartDate, filterEndDate !== defaultEndDate].filter(Boolean).length;
    ...
    <div style={s.page}>
        ...
        <div style={s.overlay} onClick={() => setShowFilter(false)}>
          <div style={s.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={s.modalHeader}>
              ...
              <button onClick={() => setShowFilter(false)} style={s.closeBtn}>
                <X size={18} />
              </button>
            </div>
            ...
            <div style={s.modalContent}>
              ...
              <div style={s.section}>
                ...
                <div style={s.presets}>
                  {datePresets.map((preset) => (
                    <button
                      ...
                      style={{
                        ...s.presetBtn,
                        background: tempFilterStartDate === new Date(new Date(defaultEndDate).setDate(new Date(defaultEndDate).getDate() - preset.days)).toISOString().split("T")[0] ? "var(--green)" : "var(--gray-100)",
                        color: tempFilterStartDate === new Date(new Date(defaultEndDate).setDate(new Date(defaultEndDate).getDate() - preset.days)).toISOString().split("T")[0] ? "#fff" : "var(--gray-700)",
                      }}
                    >
                      {preset.label}
                    </button>
                ...
                <div style={s.dateInputs}>
                  <div style={s.dateField}>
                    ...
                    <input type="date" value={tempFilterStartDate} onChange={handleDateChange(setTempFilterStartDate, setTempFilterEndDate, true)} style={s.dateInput} max={tempFilterEndDate} />
                  </div>
                  ...
                  <div style={s.dateField}>
                    ...
                    <input type="date" value={tempFilterEndDate} onChange={handleDateChange(setTempFilterEndDate, setTempFilterStartDate, false)} style={s.dateInput} min={tempFilterStartDate} max={defaultEndDate} />
                  </div>
                </div>
                ...
                <div style={s.daysHint}>
                  ...
                  <span>Maks 30 hari • Tersisa: {Math.max(0, 30 - (Math.ceil(Math.abs(new Date(tempFilterEndDate) - new Date(tempFilterStartDate)) / (1000 * 60 * 60 * 24)) + 1))} hari</span>
                </div>
              </div>
              ...
              <div style={s.section}>
                ...
                <select value={tempFilterCategory} onChange={(e) => setTempFilterCategory(e.target.value)} style={s.select}>
        ...
        <div style={s.tagsRow}>
          {filterType !== "all" && (
            <span style={s.tag}>
              ...
              <button onClick={() => setFilterType("all")} style={s.tagClose}>
                <X size={12} />
              </button>
            </span>
          )}
          {filterCategory !== "all" && categories.find((c) => c.id === filterCategory) && (
            <span style={s.tag}>
              ...
              <button onClick={() => setFilterCategory("all")} style={s.tagClose}>
                <X size={12} />
              </button>
            </span>
          )}
          {(filterStartDate !== defaultStartDate || filterEndDate !== defaultEndDate) && (
            <span style={s.tag}>
              ...
              <button onClick={clearAllFilters} style={s.tagClose}>
                <X size={12} />
              </button>
            </span>
          )}
      ...
      <div style={s.summaryRow}>
        <div style={s.summaryCard}>
          <div style={{ ...s.summaryIcon, background: "var(--green-pale)" }}>
      ...
      {loading ? (
        ...
        <div style={s.empty}>
          ...
          <p style={{ ...s.emptyText, fontSize: "13px", marginTop: "4px" }}>Coba ubah rentang tanggal atau filter lainnya</p>
                    ...
                    <div style={s.trxLeft}>
                      ...
                      <div>
                        ...
                        {dateLabel === "Hari ini" && trx.created_at && (
                          <p style={s.trxTime}>
                            {new Date(trx.created_at + "Z").toLocaleString("id-ID", {
