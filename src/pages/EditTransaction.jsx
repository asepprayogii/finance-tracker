...
export default function EditTransaction() {
  ...
  useEffect(() => { if (!fetching) fetchCategories() }, [type])
  ...
  async function handleSubmit(e) {
    ...
    const { error } = await supabase.from('transactions').update({
      type, amount: parseFloat(amount), description, date, category_id: categoryId
    }).eq('id', id)
    if (error) { setError(error.message) } else { navigate('/transactions') }
    setLoading(false)
  }

  if (fetching) return <div style={{ padding: '40px', textAlign: 'center', color: 'var(--gray-500)' }}>Memuat...</div>
