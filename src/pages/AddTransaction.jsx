...
export default function AddTransaction() {
  ...
  // Fetch categories saat mount atau tipe berubah
  useEffect(() => { 
    fetchCategories() 
  }, [type])
  ...
  async function fetchCategories() {
    ...
    try {
      ...
      setCategories(data || [])
      // Auto-select kategori pertama jika belum ada
      setCategoryId(prev => {
  ...
  async function handleSubmit(e) {
    ...
    try {
      // ✅ FIX: Destructuring yang benar untuk Supabase auth
      const { data: authData, error: authErr } = await supabase.auth.getUser()
      if (authErr || !authData?.user) throw new Error('Sesi login habis')
      const user = authData.user
  ...
  return (
    <div style={s.page}>
      {/* Header */}
      <div style={s.header}>
        <button onClick={() => navigate(-1)} style={s.iconBtn} aria-label="Kembali">
          <ChevronLeft size={20} />
        </button>
        <h2 style={s.title}>Tambah Transaksi</h2>
      ...
      <form onSubmit={handleSubmit} style={s.form}>
        ...
        <div style={s.quickAmounts}>
          {quickAmounts.map(amt => (
            <button
              ...
              style={{
                ...s.quickAmountBtn,
                background: rawAmount === String(amt) ? 'var(--gray-200)' : 'var(--gray-50)'
              }}
            >
        ...
        <button 
          ...
          style={{
            ...
            opacity: (loading || !rawAmount) ? 0.6 : 1
          }}
        >
        ...
        <div style={s.keypadOverlay} onClick={() => setShowKeypad(false)}>
          <div style={s.keypad} onClick={e => e.stopPropagation()}>
            <div style={s.keypadHeader}>
              <span style={s.keypadTitle}>Masukkan Jumlah</span>
            ...
            <div style={s.keypadGrid}>
                ...
                <div key={i} style={s.keypadRow}>
                  {row.map(key => (
                    <button
                      ...
                      style={{
                        ...s.keypadBtn,
                        background: key === 'clear' || key === 'backspace' ? 'var(--gray-100)' : key === '0' ? 'var(--gray-50)' : 'var(--white)',
                        color: key === 'clear' || key === 'backspace' ? 'var(--gray-600)' : 'var(--gray-900)',
              ...
              <button type="button" onClick={() => handleKey('done')} style={{...s.keypadBtn, ...s.keypadDone}}>
                Selesai
              </button>
            </div>
          </div>
